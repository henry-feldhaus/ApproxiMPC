...
class EnhancedSpline:
    ...
    def find_theta(self, coord: np.ndarray, theta_est, eps: float = 0.01):
        ...
        dist = np.linalg.norm(coord - np.array(self.get_coordinate(theta_est)).reshape(2))
        min_dist = dist
        min_theta = theta_est
        theta_i = theta_est
        while not found:
            theta_i += eps
            dist = np.linalg.norm(coord - np.array(self.get_coordinate(theta_i)).reshape(2))
...
class SplineTrack:
    ...
    def __init__(
        ...
        for i in range(len(params) - 1):
            ...
            coords[0, i, :] = coords[0, i, :] + safety_margin * direction / np.linalg.norm(direction + 0.001)

            # shrink out line
            direction = coords[1, i, :] - coords[2, i, :]
            coords[2, i, :] = coords[2, i, :] + safety_margin * direction / np.linalg.norm(direction + 0.001)

        # 3 set track #
        ###############
        self.refline = EnhancedSpline(coords[1, :, :], params, self.track_length)
        self.intline = EnhancedSpline(coords[0, :, :], params, self.track_length)
        self.outline = EnhancedSpline(coords[2, :, :], params, self.track_length)
        ...
        if path_to_traj is not None:
            ...
            self.delta_traj = self.trajectory.find_theta_slow(self.refline.get_coordinate(0))
        else:
...
class SplineTrackNew:
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if coords_param_direct is None:
            coords, left_widths, right_widths = read_coords_from_file_new(path_to_file)
            if params is None:
        ...
        if path_to_traj is not None:
            ...
            traj_coords = np.array(traj_coords)
            params, self.traj_len = find_params(traj_coords)
            self.trajectory = EnhancedSpline(traj_coords, params, self.track_length)
            self.delta_traj = self.trajectory.find_theta_slow(self.refline.get_coordinate(0))
        else:
            self.trajectory = self.refline
            self.traj_len = self.track_length
        ...
        if norm_vecs_x is not None and norm_vecs_y is not None:
            ...
            rot_90 = np.array([[np.cos(-np.pi / 2), -np.sin(-np.pi / 2)], [np.sin(-np.pi / 2), np.cos(-np.pi / 2)]])
    ...
    def get_frenet_coordinate(self, theta, displacement):
        ...
        if self.norms:
            norm_v = np.array([self.norm_vecs_x(theta), self.norm_vecs_y(theta)])
            # renormalize for safety
            norm_v /= np.linalg.norm(norm_v)

            return coord - norm_v * displacement
        else:
            raise NotImplementedError("Normals are not available, it is currently impossible to get frenet coordinate")
